...
export const PUBLIC_FOLDER_PATH = path.join(app.getPath("userData"), "Meridia");
...
if (!fs.existsSync(PUBLIC_FOLDER_PATH))
  fs.mkdirSync(PUBLIC_FOLDER_PATH, { recursive: true });

if (!fs.existsSync(PUBLIC_THEME_FOLDER_PATH))
  fs.mkdirSync(PUBLIC_THEME_FOLDER_PATH, { recursive: true });

if (!fs.existsSync(SETTINGS_JSON_PATH))
  fs.writeFileSync(SETTINGS_JSON_PATH, "");

if (!fs.existsSync(UI_STATE_JSON_PATH))
...
ipcMain.on("set-settings", (event, settings: IEditorSettings) => {
